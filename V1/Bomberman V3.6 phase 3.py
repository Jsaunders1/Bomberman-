...
wallblock = pygame.image.load("brickblock.png")
...
class blowrectangle(object): #Creates a class for spawning the explosions
     ...
     def update (self,screen,level):
        ...
        pygame.draw.rect(screen,(240,162,14), (self.rect)) #draws the recangle with the given colour (0,21,45)
...
class bomb(object): #This defines the bombs
      def __init__(self, posx, posy) :
       ...
       self.start = time.time()
       self.exploded = False
       self.rect = pygame.Rect((posx-20,posy-20),(30,30))
       self.active = False
       self.velX = 0
       self.velY = 0
       
      def display(self,screen):
        pygame.draw.circle(screen, (black), self.pos,15) #Sets the size and colour of the bombs and draws them
...
bombs = []
powerups = [] 
player1 = Player(40,40,pygame.K_w,pygame.K_s,pygame.K_a,pygame.K_d,pygame.K_SPACE)#creates a player using the class above
player2 = Player(1080,680,pygame.K_UP, pygame.K_DOWN, pygame.K_LEFT, pygame.K_RIGHT,pygame.K_RETURN)#Creates a second player using different inputs
colour = (0,0,0)
black = (0,0,0)
...
for row in level:
    for col in row:
        ...
        if col == "S":
            Start(x,y)
        if col == "B":
            Q = random.randint(0,124)  #Gives a 60% chance of bricks spawning                           
            if Q <= 2:
             Brick(x,y)    
